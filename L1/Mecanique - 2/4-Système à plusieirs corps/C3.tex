...
\subsection{Rappels}
\warningInfo{Conservation de l'impulsion}{Dans un système fermé, l'impulsion totale du système est conservée.}
\begin{definition}[Système élastique]
...
\criticalInfo{Quantité de mouvement dans le référentiel du centre de masse}{On a : \[\vec{P*} = M\vec{v*_c} = \vec{0}\]}
...
% \[\vec{L_O} = L_{O'} + \vec{OO'}\wedge \vec{P}\] d'où l'on déduit la relation entre le moment cinétique au point O et au centre de masse :
\[\vec{L_O} = \vec{L_C} + \vec{OC}\wedge \vec{P}\]
\end{proposition}
